...
const loginUser = asyncHandler(async (req, res) => {
  ...
  if (user && (await user.matchPassword(password))) {
    if (!user.verified) {
      ...
      console.log(
        "⚠️ [login] Bypassing email verification in development mode.",
      );
    }
...
const registerUser = asyncHandler(async (req, res) => {
  ...
  if (user) {
    ...
    if (process.env.NODE_ENV === "production") {
      ...
      console.log(
        "⚠️ [register] Skipping email verification in development mode.",
      );
    }
  } else {
    res.status(400);
...
const updateUserData = asyncHandler(async (req, res) => {
  ...
  if (updated.verified) {
    ...
    res.json({
      success: true,
      message: "Details updated, please verify new email address.",
    });
  }
});
...
// @desc    Dev: list recent users (dev only)
// route    GET /api/user/debug/list
// @access  Dev
const debugList = asyncHandler(async (req, res) => {
  if (process.env.NODE_ENV === "production") {
    res.status(403);
  ...
  const users = await User.find()
    .select("username email verified createdAt")
    .sort({ createdAt: -1 })
    .limit(20)
    .lean();
