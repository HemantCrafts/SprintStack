...
export function sendOTPEmail(emailAddress, otp) {
  ...
  transporter.sendMail(mailOptions, (error, info) => {
    if (error) {
      ...
      console.log(
        `📧 [mail] Mail sent to ${emailAddress} for 2FA [${info.response}]`,
      );
    }
  });
}
...
export function sendVerificationEmail(emailAddress, verificationLink) {
  const mailOptions = {
    from: process.env.MAIL_USERNAME,
    to: emailAddress,
    subject: "taskSync Email Verification",
    html: `
  ...
  transporter.sendMail(mailOptions, (error, info) => {
    if (error) {
      ...
      console.log(
        `📧 [mail] Mail sent to ${emailAddress} for Verification [${info.response}]`,
      );
    }
  });
}
...
export function sendBoardInvite(emailAddress, boardName, inviteLink) {
  ...
  transporter.sendMail(mailOptions, (error, info) => {
    if (error) {
      ...
      console.log(
        `📧 [mail] Mail sent to ${emailAddress} for Collaboration [${info.response}]`,
      );
    }
  });
}
